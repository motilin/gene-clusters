...
export const getMatrix = async (req, res) => {
  const mt = await db.collection(`matrix${suffix}`);
  const cursor = await mt.find({});
  cursor.stream().pipe(JSONStream.stringify()).pipe(res);
};

export const getFlattened = async (req, res) => {
  const fl = await db.collection(`flattened${suffix}`);
  const cursor = await fl.find({});
  cursor.stream().pipe(JSONStream.stringify()).pipe(res);
};
